 ...
 * 悬挂的山楂方块
 * <p>
 * 继承自悬挂果实基类，实现山楂的生长和采集逻辑
 * 山楂成熟后（AGE=4）可以被玩家右键采集，掉落2个山楂，采集后重置生长阶段
 */
public class HangingHawthornBlock extends HangingFruitBlock {
    /**
     * 构造悬挂山楂方块
     *
     * @param settings 方块属性配置
     */
    public HangingHawthornBlock(Properties settings) {
        super(settings);
    }
    ...
    public boolean canSurvive(BlockState state,LevelReader world, BlockPos pos) {
        ...
        return !soilDecision.isDefault() ? soilDecision.isTrue() : world.getBlockState(pos.above()).is(FruityBlocks.FLOWER_HAWTHORN_LEAVES);
    }
    ...
    public InteractionResult useWithoutItem(BlockState state, Level world, BlockPos pos, Player player, BlockHitResult hit) {
        ...
        if (i == 4) {
            HangingFruitBlock.popResource(world, pos, new ItemStack(FruityItems.HAWTHORN.get(), 2));
     ...
     * @return 包含山楂物品的 ItemStack
     */
    @Override
    public ItemStack getCloneItemStack(BlockState state, HitResult target, LevelReader level, BlockPos pos, Player player) {
        return new ItemStack(FruityItems.HAWTHORN.get());
    }
}
