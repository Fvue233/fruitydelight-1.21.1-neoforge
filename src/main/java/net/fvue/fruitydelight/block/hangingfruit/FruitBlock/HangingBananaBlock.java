 ...
 * 悬挂的香蕉方块
 * <p>
 * 继承自悬挂果实基类，实现香蕉的生长和采集逻辑
 * 香蕉成熟后（AGE=4）可以被玩家右键采集，掉落3个香蕉串，采集后重置生长阶段
 */
public class HangingBananaBlock extends HangingFruitBlock {
    ...
    public VoxelShape getShape(BlockState state, BlockGetter world, BlockPos pos, CollisionContext context) {
        VoxelShape voxelShape = HangingFruitShapes.BANANA_SHAPES[state.getValue(AGE)];
    ...
    public VoxelShape getCollisionShape(BlockState state, BlockGetter world, BlockPos pos, CollisionContext context) {
        VoxelShape voxelShape = HangingFruitShapes.BANANA_COLL_SHAPES[state.getValue(AGE)];
        Vec3 vec3d = state.getOffset(world, pos);
        return voxelShape.move(vec3d.x, vec3d.y, vec3d.z);
    }
     ...
     * @return 包含香蕉物品的 ItemStack
     */
    @Override
    public ItemStack getCloneItemStack(BlockState state, HitResult target, LevelReader level, BlockPos pos, Player player) {
        return new ItemStack(FruityItems.BANANA.get());
    }
}
