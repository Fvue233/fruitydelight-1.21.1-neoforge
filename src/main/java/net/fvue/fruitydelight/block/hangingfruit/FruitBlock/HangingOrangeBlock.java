 ...
 * 悬挂的橙子方块
 * <p>
 * 继承自悬挂果实基类，实现橙子的生长和采集逻辑
 * 橙子成熟后（AGE=4）可以被玩家右键采集，掉落1个橙子，采集后重置生长阶段
 */
public class HangingOrangeBlock extends HangingFruitBlock {
    ...
    public boolean canSurvive(BlockState state,LevelReader world, BlockPos pos) {
        ...
        return !soilDecision.isDefault() ? soilDecision.isTrue() : world.getBlockState(pos.above()).is(FruityBlocks.FLOWER_ORANGE_LEAVES);
    }
    ...
    public  InteractionResult useWithoutItem(BlockState state,  Level world,  BlockPos pos,  Player player,  BlockHitResult hit) {
        ...
        if (i == 4) {
            HangingFruitBlock.popResource(world, pos, new ItemStack(FruityItems.ORANGE.get(), 1));
     ...
     * @return 包含橙子物品的 ItemStack
     */
    @Override
    public ItemStack getCloneItemStack(BlockState state, HitResult target, LevelReader level, BlockPos pos, Player player) {
        return new ItemStack(FruityItems.ORANGE.get());
    }
}
