 ...
 */
public final class AppleCakeBlock extends CakeBlock {
    /**
     * 构造梨蛋糕方块
     * 复制原版蛋糕的方块属性
     */
    ...
    protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        ...
        // 生成苹果蛋糕切片物品
        final var cake = new ItemStack(FruityItems.APPLE_CAKE_SLICE.get());

        // 尝试添加到玩家背包，失败则掉落在地上
        if (!player.addItem(cake)) {
            level.addFreshEntity(new ItemEntity(level, pos.getX(), pos.getY(), pos.getZ(), cake));
        }
